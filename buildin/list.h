...
#define list_for_each_entry_safe_t(pos, n, head, t, member)			\
	for (pos = list_entry((head)->next, t, member),	\
...
#include <memory>
/**
 * class T must define a member named 'node' as struct list_head
 */
template<class T>
std::unique_ptr<T,std::function<void(T*)>> list_node_new_auto_t( struct list_head *h)
{
	...
	auto p_ptr = list_entry(p, T, node);
	auto ps = std::unique_ptr<T,std::function<void(T*)> >(p_ptr, [p,h](T*v)
	{
		list_add_tail( p, h);
	});
	return ps;
...
#ifdef __cplusplus
template<class T>
T* list_node_alloc( struct list_head *h)
{
	struct list_head *p = 0;
	if (list_empty (h))
	{
...
template<class T>
void list_node_free( struct list_head *h, T *p)
{
	list_del_init(&p->node);
	list_add_tail(&p->node, h);
}
#endif
