    ...
    while (open_.size() > 0) {
        ...
        if (current.ws_.meetsGoal(goal)) {
            ...
            do {
                ...
                auto itr = std::find_if(begin(open_), end(open_), [&current](const Node & n) { return n.id_ == current.parent_id_; });
                if (itr == end(open_)) {
                    itr = std::find_if(begin(closed_), end(closed_), [&current](const Node & n) { return n.id_ == current.parent_id_; });
                }
                current = *itr;
