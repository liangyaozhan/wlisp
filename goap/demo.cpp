...
int demo0(void) {
	...
	Action snotice("Notice", 0);
	snotice.setPrecondition(notice, false);
	snotice.setEffect(notice, true);
	actions.push_back(snotice);
    ...
    goal_win.setVariable(notice, true);
    ...
    initial_state.setVariable(inventory_knife, false);
    initial_state.setVariable(inventory_gun, true);
    initial_state.setVariable(notice, false);

	auto dump = [&]() {
		std::cout << "***********************************\n";
#define XXX(x) std::cout << #x "=" << initial_state.getVariable(x) << "\n";
		XXX(enemy_dead);
		XXX(enemy_sighted);
		XXX(enemy_in_range);
		XXX(enemy_in_close_range);
		XXX(gun_loaded);
		XXX(gun_drawn);
		XXX(knife_drawn);
		XXX(weapon_in_hand);
    ...
    try {
        ...
        for (std::vector<Action>::reverse_iterator rit = the_plan.rbegin(); rit != the_plan.rend(); ++rit) {
            std::cout << rit->name() << "\n";
			initial_state = rit->actOn(initial_state);
			dump();
        }
